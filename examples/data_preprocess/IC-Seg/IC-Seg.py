...
def make_map_fn(split, num_frames, max_size, min_size, data_dir, model_type):
    def process_fn(example, idx):
        ...
        system_prompt = search_prompt if ability in ["difficult", "medium", "simple"] else non_search_prompt
        ...
        visual_dir = os.path.join(data_dir, "Visualization", video_dir, str(oid))
        visual_paths = [os.path.join(visual_dir, image+'.jpg') for image in image_names]
        ...
        mm_content = ''
        for i in indices:
            fid = int(image_names[i].split('_')[-1])
            images.append(image_paths[i])
            visual.append(visual_paths[i])
        ...
        target_path = os.path.join(data_dir, "description_oid", video_dir, f"{int(oid)}.json")

        mm_content += f'\n{prompt}'
        data = {
            "data_source": f"{ability}",
        ...
        objects = example.get('object', '')
        if objects:
            data['reward_model']['object'] = objects
        
        max_count = example.get('max_count', -1)
        if max_count != -1:
            data['reward_model']['max_count'] = max_count

        initial_count = example.get('initial_count', -1)
        if initial_count != -1:
            data['reward_model']['initial_count'] = initial_count
        
        if split == 'train':
            aid = example['aid']
            data['extra_info']['all_target_path'] = os.path.join(data_dir, "description_id", f"{int(aid)}.json")
...
def main(local_parquet_filepath, local_save_dir, split="train", num_frames=6, max_size=640, min_size=352, model_type="Qwen3VL"):
    ...
    data_dir = data_dir / 'RVOS_Train' if split == 'train' else data_dir / 'RVOS_Test'
    dataset = df_raw.map(
...
if __name__ == "__main__":
    ...
    main(
        local_parquet_filepath=f'data/IC-Seg/{args.split}.parquet',
        local_save_dir="data/IC-Seg",
        split=args.split,
