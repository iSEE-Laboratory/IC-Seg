...
val_data=[$(pwd)/data/IC-Seg/test_6_448_448.parquet]
...
# clear the folderre
rm -rf $(pwd)/results/${run_name}_eval
echo "Deleted previous eval folders: results/${run_name}_eval"
...
val_repetition_penalty=${VAL_REPETITION_PENALTY:-$repetition_penalty}
presence_penalty=${PRESENCE_PENALTY:-1.0}
val_presence_penalty=${VAL_PRESENCE_PENALTY:-$presence_penalty}
enable_agent=True # enable agent for tool use
strategy="fsdp2"
action_stop_tokens="</call>,</keyframe>"
max_turns=6
reward_manager=ic_seg_val
...
python -m verl_tool.servers.serve --host $host --port $port --tool_type "vlm_tool,keyframe" --workers_per_tool 8 &
server_pid=$!
